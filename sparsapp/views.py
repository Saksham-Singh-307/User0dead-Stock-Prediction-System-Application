...
from .models import Project , profile as pr , otp 
...
def index(request,backend = 'django.contrib.auth.backends.ModelBackend'):
    ...
    df1 = yf.download(tickers = 'AAPL', period='1d', interval='1mo')
    df2 = yf.download(tickers = 'AMZN', period='1d', interval='1mo')
    df3 = yf.download(tickers = 'GOOGL', period='1d', interval='1mo')
    df4 = yf.download(tickers = 'UBER', period='1d', interval='1mo')
    df5 = yf.download(tickers = 'TSLA', period='1d', interval='1mo')
    df6 = yf.download(tickers = 'SMCI', period='1d', interval='1mo')
    ...
    df6.insert(0, "Ticker", "SMCI")

    df = pd.concat([df1, df2, df3, df4, df5, df6], axis=0)
    df.reset_index(level=0, inplace=True)
    df.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31']
    c=['Ticker',  'open' , 'High', 'Low','Close', 'Adj_Close']
    d=[]
    g=[]
    p=[]
    j=[]
    ...
    r=list(zip(d,g,p,j,k,m))
    fd=pd.DataFrame(r,columns = c)
    ...
    json_records = fd.reset_index().to_json(orient ='records')
    recent_stocks = []
...
def predict(request, ticker_value, number_of_days):
    ...
    ticker = pd.read_csv('sparsapp/Data/Tickers.csv')
...
@csrf_exempt
def logged(request,backend = 'django.contrib.auth.backends.ModelBackend'):
    a=request.POST.get("luser")
    b=request.POST.get("lpass")
    user=authenticate(request,username=a,password=b)
    if user is not None:
        lg(request, user)
        return index(request)
    else:
        messages.error(request, ("ENTER THE CORRECT USERNAME AND PASSWORD"))
        return render(request,'login.html')

@csrf_exempt
def signup(request):
    a=request.POST.get("suser")
    b=request.POST.get("semail")
    c=request.POST.get("spass")
    forbidden_users = ['admin', 'css', 'js', 'authenticate', 'login', 'logout', 'administrator', 'root',
    'email', 'user', 'join', 'sql', 'static', 'python', 'delete']
    if a.lower() in forbidden_users:
    ...
    elif User.objects.filter(email__iexact=b).exists():
        raise ValidationError('User with this email already exists.')
    elif User.objects.filter(username__iexact=a).exists():
...
def ticker(request):
    ticker_df = pd.read_csv('sparsapp/Data/new_tickers.csv') 
    json_ticker = ticker_df.reset_index().to_json(orient ='records')
    ticker_list = []
    ticker_list = json.loads(json_ticker)


    return render(request,'try.html',{'ticker_list': ticker_list,
...
def updated(request):
    if request.method =='POST':
        ...
        c=request.POST.get("femail")
        d=request.POST.get("bio")
        

        pro=pr.objects.get(user=user)
        profile_image=request.FILES.get("profile_pic")
        pro.profile_image=profile_image
        pro.profile_bio=d
...
def market(request):
    def get_company_name(ticker_symbol):
        try:
            ...
            # Extract the full name of the company
            full_name = company_info.get('longName', 'Company name not found')
    ...
    df1 = yf.download(tickers = 'AAPL', period='1d', interval='1mo')
    df2 = yf.download(tickers = 'MSFT', period='1d', interval='1mo')
    df3 = yf.download(tickers = 'AMZN', period='1d', interval='1mo')
    df4 = yf.download(tickers = 'GOOGL', period='1d', interval='1mo')
    df5 = yf.download(tickers = 'TSLA', period='1d', interval='1mo')
    df6 = yf.download(tickers = 'META', period='1d', interval='1mo')
    df7 = yf.download(tickers = 'NVDA', period='1d', interval='1mo')
    df8 = yf.download(tickers = 'BABA', period='1d', interval='1mo')
    df9 = yf.download(tickers = 'SSNLF', period='1d', interval='1mo')
    df10 = yf.download(tickers = 'INTC', period='1d', interval='1mo')
    df11 = yf.download(tickers = 'A', period='1d', interval='1mo')
    df12 = yf.download(tickers = 'AA', period='1d', interval='1mo')
    df13 = yf.download(tickers = 'BBD', period='1d', interval='1mo')
    df14 = yf.download(tickers = 'PLTR', period='1d', interval='1mo')
    df15 = yf.download(tickers = 'SOUN', period='1d', interval='1mo')
    df16 = yf.download(tickers = 'SOFI', period='1d', interval='1mo')
    df17 = yf.download(tickers = 'GRAB', period='1d', interval='1mo')
    df18 = yf.download(tickers = 'VALE', period='1d', interval='1mo')
    df19 = yf.download(tickers = 'NIO', period='1d', interval='1mo')
    df20 = yf.download(tickers = 'ITUB', period='1d', interval='1mo')

    df1.insert(0, "Ticker", "AAPL")
    df2.insert(0, "Ticker", "MSFT")
    df3.insert(0, "Ticker", "AMZN")
    df4.insert(0, "Ticker", "GOOGL")
    df5.insert(0, "Ticker", "TSLA")
    df6.insert(0, "Ticker", "META")
    df7.insert(0, "Ticker", "NVDA")
    df8.insert(0, "Ticker", "BABA")
    df9.insert(0, "Ticker", "SSNLF")
    df10.insert(0, "Ticker", "INTC")
    df11.insert(0, "Ticker", "A")
    df12.insert(0, "Ticker", "AA")
    df13.insert(0, "Ticker", "BBD")
    df14.insert(0, "Ticker", "PLTR")
    df15.insert(0, "Ticker", "SOUN")
    df16.insert(0, "Ticker", "SOFI")
    df17.insert(0, "Ticker", "GRAB")
    df18.insert(0, "Ticker", "VALE")
    df19.insert(0, "Ticker", "NIO")
    df20.insert(0, "Ticker", "ITUB")

    df = pd.concat([df1, df2, df3, df4, df5, df6, df7 ,df8 , df9 ,df10, df11 ,df12 ,df13 ,df14 ,df15 ,df16 ,df17 ,df18 ,df19 ,df20 ], axis=0)
    df.reset_index(level=0, inplace=True)
    df.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35']
    c=['Ticker', 'open','High','Low','Close','Adj Close','change','company_name']
    d=[]
    g=[]
    p=[]
    j=[]
    ...
    for i in range(0,20):
        ...
        if i==9:
            rs=list(zip(d,g,p,j,k,m,s,na))
            fd=pd.DataFrame(rs,columns = c)

    r=list(zip(d,g,p,j,k,m,s,na))
    ld=pd.DataFrame(r,columns = c)

    bdf1 = yf.download(tickers = 'SURI', period='1d', interval='1mo')
    bdf2 = yf.download(tickers = 'SMCX', period='1d', interval='1mo')
    bdf3 = yf.download(tickers = 'CONL', period='1d', interval='1mo')
    bdf4 = yf.download(tickers = 'KOLD', period='1d', interval='1mo')
    bdf5 = yf.download(tickers = 'BRZU', period='1d', interval='1mo')
    bdf6 = yf.download(tickers = 'UBR', period='1d', interval='1mo')
    bdf7 = yf.download(tickers = 'TSLQ', period='1d', interval='1mo')
    bdf8 = yf.download(tickers = 'TSLZ', period='1d', interval='1mo')
    bdf9 = yf.download(tickers = 'TSDD', period='1d', interval='1mo')
    bdf10 = yf.download(tickers = 'EWZS', period='1d', interval='1mo')
    bdf1.insert(0, "Ticker", "SURI")
    bdf2.insert(0, "Ticker", "SMCX")
    bdf3.insert(0, "Ticker", "CONL")
    bdf4.insert(0, "Ticker", "KOLD")
    bdf5.insert(0, "Ticker", "BRZU")
    bdf6.insert(0, "Ticker", "UBR")
    bdf7.insert(0, "Ticker", "TSLQ")
    bdf8.insert(0, "Ticker", "TSLZ")
    bdf9.insert(0, "Ticker", "TSDD")
    bdf10.insert(0, "Ticker", "EWZS")
    bdf = pd.concat([bdf1, bdf2, bdf3, bdf4, bdf5, bdf6 , bdf7 , bdf8 , bdf9 , bdf10 ], axis=0)
    bdf.reset_index(level=0, inplace=True)    
    bdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','col_38','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37']
    bc=['Ticker','open','High','Low','Close','Adj Close','change','company_name']
    bd=[]
    bg=[]
    bp=[]
    bj=[]
    ...
    br=list(zip(bd,bg,bp,bj,bk,bm,bs,bna))
    bfd=pd.DataFrame(br,columns = bc)
    bfd

    edf1 = yf.download(tickers = 'AAPL', period='1d', interval='1mo')
    edf2 = yf.download(tickers = 'MSFT', period='1d', interval='1mo')
    edf3 = yf.download(tickers = 'AMZN', period='1d', interval='1mo')
    edf4 = yf.download(tickers = 'GOOGL', period='1d', interval='1mo')
    edf5 = yf.download(tickers = 'TSLA', period='1d', interval='1mo')
    edf6 = yf.download(tickers = 'META', period='1d', interval='1mo')
    edf7 = yf.download(tickers = 'NVDA', period='1d', interval='1mo')
    edf8 = yf.download(tickers = 'ADBE', period='1d', interval='1mo')
    edf9 = yf.download(tickers = 'PYPL', period='1d', interval='1mo')
    edf10 = yf.download(tickers = 'INTC', period='1d', interval='1mo')
    edf11 = yf.download(tickers = 'SNAP', period='1d', interval='1mo')
    edf12 = yf.download(tickers = 'SURI', period='1d', interval='1mo')
    edf13 = yf.download(tickers = 'PTON', period='1d', interval='1mo')
    edf14 = yf.download(tickers = 'ZM', period='1d', interval='1mo')
    edf15 = yf.download(tickers = 'HOOD', period='1d', interval='1mo')
    edf16 = yf.download(tickers = 'GME', period='1d', interval='1mo')
    edf17 = yf.download(tickers = 'BB', period='1d', interval='1mo')
    edf18 = yf.download(tickers = 'IBM', period='1d', interval='1mo')
    edf19 = yf.download(tickers = 'DELL', period='1d', interval='1mo')
    edf20 = yf.download(tickers = 'SSNLF', period='1d', interval='1mo')

    edf1.insert(0, "Ticker", "AAPL")
    edf2.insert(0, "Ticker", "MSFT")
    edf3.insert(0, "Ticker", "AMZN")
    edf4.insert(0, "Ticker", "GOOGL")
    edf5.insert(0, "Ticker", "TSLA")
    edf6.insert(0, "Ticker", "META")
    edf7.insert(0, "Ticker", "NVDA")
    edf8.insert(0, "Ticker", "ADBE")
    edf9.insert(0, "Ticker", "PYPL")
    edf10.insert(0, "Ticker", "INTC")
    edf11.insert(0, "Ticker", "SNAP")
    edf12.insert(0, "Ticker", "SURI")
    edf13.insert(0, "Ticker", "PTON")
    edf14.insert(0, "Ticker", "ZM")
    edf15.insert(0, "Ticker", "HOOD")
    edf16.insert(0, "Ticker", "GME")
    edf17.insert(0, "Ticker", "BB")
    edf18.insert(0, "Ticker", "IBM")
    edf19.insert(0, "Ticker", "DELL")
    edf20.insert(0, "Ticker", "SSNLF")

    edf = pd.concat([edf1, edf2, edf3, edf4, edf5, edf6, edf7 ,edf8 , edf9 ,edf10, edf11 ,edf12 ,edf13 ,edf14 ,edf15 ,edf16 ,edf17 ,edf18 ,edf19 ,edf20 ], axis=0)
    edf.reset_index(level=0, inplace=True)
    bedf = pd.concat([edf11 ,edf12 ,edf13 ,edf14 ,edf15 ,edf16 ,edf17 ,edf18 ,edf19 ,edf20 ], axis=0)
    bedf.reset_index(level=0, inplace=True)
    bedf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','col_38','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37']
        

    edf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35']
    c=['Ticker','open','High','Low','Close','Adj Close','change','company_name']
    ed=[]
    eg=[]
    ep=[]
    ...
    for i in range(0,10):
        ...
        if i==9:
            ers=list(zip(ed,eg,ep,ej,ek,em,es,ena))
            efd=pd.DataFrame(ers,columns = c)
    for i in range(0,10):
        bedata=bee[i][1]
        bedac=bee[i][beac]
        bedc=bee[i][becl]
        ...
        bedch= bedc-bedo
        bename=get_company_name(bedata)
        bed.append(bedata)
        beg.append(bedac)
        bep.append(bedc)
        bej.append(bedh)
    ...
    ber=list(zip(bed,beg,bep,bej,bek,bem,bes,bena))
    beld=pd.DataFrame(ber,columns = c)
    beld

    # -- HEALTH SECTOR CODE --//
    Hdf1 = yf.download(tickers = 'DVA', period='1d', interval='1mo')
    Hdf2 = yf.download(tickers = 'BSX', period='1d', interval='1mo')
    Hdf3 = yf.download(tickers = 'JNJ', period='1d', interval='1mo')
    Hdf4 = yf.download(tickers = 'EW', period='1d', interval='1mo')
    Hdf5 = yf.download(tickers = 'UNH', period='1d', interval='1mo')
    Hdf6 = yf.download(tickers = 'VEEV', period='1d', interval='1mo')
    Hdf7 = yf.download(tickers = 'JANX', period='1d', interval='1mo')
    Hdf8 = yf.download(tickers = 'CGC', period='1d', interval='1mo')
    Hdf9 = yf.download(tickers = 'TLRY', period='1d', interval='1mo')
    Hdf10 = yf.download(tickers = 'AMGN', period='1d', interval='1mo')
    Hdf11 = yf.download(tickers = 'SGIOF', period='1d', interval='1mo')
    Hdf12 = yf.download(tickers = 'AMZN', period='1d', interval='1mo')
    Hdf13 = yf.download(tickers = 'VRNOF', period='1d', interval='1mo')
    Hdf14 = yf.download(tickers = 'MRNA', period='1d', interval='1mo')
    Hdf15 = yf.download(tickers = 'BMRN', period='1d', interval='1mo')
    Hdf16 = yf.download(tickers = 'GMAB', period='1d', interval='1mo')
    Hdf17 = yf.download(tickers = 'NEU', period='1d', interval='1mo')
    Hdf18 = yf.download(tickers = 'IDXX', period='1d', interval='1mo')
    Hdf19 = yf.download(tickers = 'CNTGF', period='1d', interval='1mo')
    Hdf20 = yf.download(tickers = 'HUM', period='1d', interval='1mo')

    Hdf1.insert(0, "Ticker", "DVA")
    Hdf2.insert(0, "Ticker", "BSX")
    Hdf3.insert(0, "Ticker", "JNJ")
    Hdf4.insert(0, "Ticker", "EW")
    Hdf5.insert(0, "Ticker", "UNH")
    Hdf6.insert(0, "Ticker", "VEEV")
    Hdf7.insert(0, "Ticker", "JANX")
    Hdf8.insert(0, "Ticker", "CGC")
    Hdf9.insert(0, "Ticker", "TLRY")
    Hdf10.insert(0, "Ticker", "AMGN")
    Hdf11.insert(0, "Ticker", "SGIOF")
    Hdf12.insert(0, "Ticker", "AMZN")
    Hdf13.insert(0, "Ticker", "VRNOF")
    Hdf14.insert(0, "Ticker", "MRNA")
    Hdf15.insert(0, "Ticker", "BMRN")
    Hdf16.insert(0, "Ticker", "GMAB")
    Hdf17.insert(0, "Ticker", "NEU")
    Hdf18.insert(0, "Ticker", "IDXX")
    Hdf19.insert(0, "Ticker", "CNTGF")
    Hdf20.insert(0, "Ticker", "HUM")

    Hdf = pd.concat([Hdf1, Hdf2, Hdf3, Hdf4, Hdf5, Hdf6, Hdf7 ,Hdf8 , Hdf9 ,Hdf10, Hdf11 ,Hdf12 ,Hdf13 ,Hdf14 ,Hdf15 ,Hdf16 ,Hdf17 ,Hdf18 ,Hdf19 ,Hdf20 ], axis=0)
    Hdf.reset_index(level=0, inplace=True)
    bHdf = pd.concat([Hdf11 ,Hdf12 ,Hdf13 ,Hdf14 ,Hdf15 ,Hdf16 ,Hdf17 ,Hdf18 ,Hdf19 ,Hdf20 ], axis=0)
    bHdf.reset_index(level=0, inplace=True)
    bHdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','col_38','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37']
        

    Hdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35']
    c=['Ticker','open','High','Low','Close','Adj Close','change','company_name']
    Hd=[]
    Hg=[]
    Hp=[]
    ...
    for i in range(0,10):
        ...
        if i==9:
            Hrs=list(zip(Hd,Hg,Hp,Hj,Hk,Hm,Hs,Hna))
            Hfd=pd.DataFrame(Hrs,columns = c)
    for i in range(0,10):
        bHdata=bHe[i][1]
        bHdac=bHe[i][bHac]
        bHdc=bHe[i][bHcl]
        ...
        bHdch= bHdc-bHdo
        bHname=get_company_name(bHdata)
        bHd.append(bHdata)
        bHg.append(bHdac)
        bHp.append(bHdc)
        bHj.append(bHdh)
    ...
    bHr=list(zip(bHd,bHg,bHp,bHj,bHk,bHm,bHs,bHna))
    bHld=pd.DataFrame(bHr,columns = c)
    Hfd

    #-- FINANCIAL SERVICE DATAFRAME --#
    Fdf1 = yf.download(tickers = 'KKR', period='1d', interval='1mo')
    Fdf2 = yf.download(tickers = 'JPM', period='1d', interval='1mo')
    Fdf3 = yf.download(tickers = 'SPGI', period='1d', interval='1mo')
    Fdf4 = yf.download(tickers = 'AJG', period='1d', interval='1mo')
    Fdf5 = yf.download(tickers = 'BLK', period='1d', interval='1mo')
    Fdf6 = yf.download(tickers = 'GS', period='1d', interval='1mo')
    Fdf7 = yf.download(tickers = 'MS', period='1d', interval='1mo')
    Fdf8 = yf.download(tickers = 'AXP', period='1d', interval='1mo')
    Fdf9 = yf.download(tickers = 'C', period='1d', interval='1mo')
    Fdf10 = yf.download(tickers = 'VCTR', period='1d', interval='1mo')
    Fdf11 = yf.download(tickers = 'KEY', period='1d', interval='1mo')
    Fdf12 = yf.download(tickers = 'SIEB', period='1d', interval='1mo')
    Fdf13 = yf.download(tickers = 'RF', period='1d', interval='1mo')
    Fdf14 = yf.download(tickers = 'FRCB', period='1d', interval='1mo')
    Fdf15 = yf.download(tickers = 'OXLC', period='1d', interval='1mo')
    Fdf16 = yf.download(tickers = 'WAL', period='1d', interval='1mo')
    Fdf17 = yf.download(tickers = 'ZION', period='1d', interval='1mo')
    Fdf18 = yf.download(tickers = 'CME', period='1d', interval='1mo')
    Fdf19 = yf.download(tickers = 'HOOD', period='1d', interval='1mo')
    Fdf20 = yf.download(tickers = 'COIN', period='1d', interval='1mo')

    Fdf1.insert(0, "Ticker", "KKR")
    Fdf2.insert(0, "Ticker", "JPM")
    Fdf3.insert(0, "Ticker", "SPGI")
    Fdf4.insert(0, "Ticker", "AJG")
    Fdf5.insert(0, "Ticker", "BLK")
    Fdf6.insert(0, "Ticker", "GS")
    Fdf7.insert(0, "Ticker", "MS")
    Fdf8.insert(0, "Ticker", "AXP")
    Fdf9.insert(0, "Ticker", "C")
    Fdf10.insert(0, "Ticker", "VCTR")
    Fdf11.insert(0, "Ticker", "KEY")
    Fdf12.insert(0, "Ticker", "SIEB")
    Fdf13.insert(0, "Ticker", "RF")
    Fdf14.insert(0, "Ticker", "FRCB")
    Fdf15.insert(0, "Ticker", "OXLC")
    Fdf16.insert(0, "Ticker", "WAL")
    Fdf17.insert(0, "Ticker", "ZION")
    Fdf18.insert(0, "Ticker", "CME")
    Fdf19.insert(0, "Ticker", "HOOD")
    Fdf20.insert(0, "Ticker", "COIN")

    Fdf = pd.concat([Fdf1, Fdf2, Fdf3, Fdf4, Fdf5, Fdf6, Fdf7 ,Fdf8 , Fdf9 ,Fdf10, Fdf11 ,Fdf12 ,Fdf13 ,Fdf14 ,Fdf15 ,Fdf16 ,Fdf17 ,Fdf18 ,Fdf19 ,Fdf20 ], axis=0)
    Fdf.reset_index(level=0, inplace=True)
    bFdf = pd.concat([Fdf11 ,Fdf12 ,Fdf13 ,Fdf14 ,Fdf15 ,Fdf16 ,Fdf17 ,Fdf18 ,Fdf19 ,Fdf20 ], axis=0)
    bFdf.reset_index(level=0, inplace=True)
    bFdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','col_38','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37']
        

    Fdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35']
    c=['Ticker', 'open','High','Low','Close','Adj Close','change','company_name']
    Fd=[]
    Fg=[]
    Fp=[]
    ...
    for i in range(0,10):
        ...
        if i==9:
            Frs=list(zip(Fd,Fg,Fp,Fj,Fk,Fm,Fs,Fna))
            Ffd=pd.DataFrame(Frs,columns = c)
    for i in range(0,10):
        bFdata=bFe[i][1]
        bFdac=bFe[i][bFac]
        bFdc=bFe[i][bFcl]
        ...
        bFdch= bFdc-bFdo
        bFname=get_company_name(bFdata)
        bFd.append(bFdata)
        bFg.append(bFdac)
        bFp.append(bFdc)
        bFj.append(bFdh)
        ...
        bFo=bFo+5
        bFv=bFv+5
    bFr=list(zip(bFd,bFg,bFp,bFj,bFk,bFm,bFs,bFna))
    bFld=pd.DataFrame(bFr,columns = c)
    Ffd

    #-- Communication servive stocks dataframe --\\

    Cdf1 = yf.download(tickers = 'META', period='1d', interval='1mo')
    Cdf2 = yf.download(tickers = 'GOOGL', period='1d', interval='1mo')
    Cdf3 = yf.download(tickers = 'NFLX', period='1d', interval='1mo')
    Cdf4 = yf.download(tickers = 'DIS', period='1d', interval='1mo')
    Cdf5 = yf.download(tickers = 'CMCSA', period='1d', interval='1mo')
    Cdf6 = yf.download(tickers = 'CHTR', period='1d', interval='1mo')
    Cdf7 = yf.download(tickers = 'TMUS', period='1d', interval='1mo')
    Cdf8 = yf.download(tickers = 'WBD', period='1d', interval='1mo')
    Cdf9 = yf.download(tickers = 'EA', period='1d', interval='1mo')
    Cdf10 = yf.download(tickers = 'TTWO', period='1d', interval='1mo')
    Cdf11 = yf.download(tickers = 'AMC', period='1d', interval='1mo')
    Cdf12 = yf.download(tickers = 'IQ', period='1d', interval='1mo')
    Cdf13 = yf.download(tickers = 'CRTO', period='1d', interval='1mo')
    Cdf14 = yf.download(tickers = 'PLTK', period='1d', interval='1mo')
    Cdf15 = yf.download(tickers = 'IRDM', period='1d', interval='1mo')
    Cdf16 = yf.download(tickers = 'ATHM', period='1d', interval='1mo')
    Cdf17 = yf.download(tickers = 'YELP', period='1d', interval='1mo')
    Cdf18 = yf.download(tickers = 'IAS', period='1d', interval='1mo')
    Cdf19 = yf.download(tickers = 'MGNI', period='1d', interval='1mo')
    Cdf20 = yf.download(tickers = 'AMZN', period='1d', interval='1mo')

    Cdf1.insert(0, "Ticker", "META")
    Cdf2.insert(0, "Ticker", "GOOGL")
    Cdf3.insert(0, "Ticker", "NFLX")
    Cdf4.insert(0, "Ticker", "DIS")
    Cdf5.insert(0, "Ticker", "CMCSA")
    Cdf6.insert(0, "Ticker", "CHTR")
    Cdf7.insert(0, "Ticker", "TMUS")
    Cdf8.insert(0, "Ticker", "WBD")
    Cdf9.insert(0, "Ticker", "EA")
    Cdf10.insert(0, "Ticker", "TTWO")
    Cdf11.insert(0, "Ticker", "AMC")
    Cdf12.insert(0, "Ticker", "IQ")
    Cdf13.insert(0, "Ticker", "CRTO")
    Cdf14.insert(0, "Ticker", "PLTK")
    Cdf15.insert(0, "Ticker", "IRDM")
    Cdf16.insert(0, "Ticker", "ATHM")
    Cdf17.insert(0, "Ticker", "YELP")
    Cdf18.insert(0, "Ticker", "IAS")
    Cdf19.insert(0, "Ticker", "MGNI")
    Cdf20.insert(0, "Ticker", "AMZN")
    Cdf = pd.concat([Cdf1, Cdf2, Cdf3, Cdf4, Cdf5, Cdf6, Cdf7 ,Cdf8 , Cdf9 ,Cdf10, Cdf11 ,Cdf12 ,Cdf13 ,Cdf14 ,Cdf15 ,Cdf16 ,Cdf17 ,Cdf18 ,Cdf19 ,Cdf20 ], axis=0)
    Cdf.reset_index(level=0, inplace=True)
    bCdf = pd.concat([Cdf11 ,Cdf12 ,Cdf13 ,Cdf14 ,Cdf15 ,Cdf16 ,Cdf17 ,Cdf18 ,Cdf19 ,Cdf20 ], axis=0)
    bCdf.reset_index(level=0, inplace=True)
    bCdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','col_38','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37','col_37']
        

    Cdf.columns = ['Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8S','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_29','col_30','col_31','col_32','col_33','col_34','col_35','col_36','col_37','Date', 'Ticker', 'Open', 'High', 'Low', 'Close', 'Adj_Close', 'Volume','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_9','col_10','col_11','col_12','col_13','col_14','col_15','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_8','col_16','col_17','col_18','col_19','col_20','col_21','col_22','col_23','col_24','col_25','col_26','col_27','col_28','col_29','col_30','col_31','col_32','col_33','col_34','col_35']
    c=['Ticker','open','High','Low','Close','Adj Close','change','company_name']
    Cd=[]
    Cg=[]
    Cp=[]
    ...
    for i in range(0,10):
        ...
        if i==9:
            Crs=list(zip(Cd,Cg,Cp,Cj,Ck,Cm,Cs,Cna))
            Cfd=pd.DataFrame(Crs,columns = c)
    for i in range(0,10):
        bCdata=bCe[i][1]
        bCdac=bCe[i][bCac]
        bCdc=bCe[i][bCcl]
        ...
        bCdch= bCdc-bCdo
        bCname=get_company_name(bCdata)
        bCd.append(bCdata)
        bCg.append(bCdac)
        bCp.append(bCdc)
        bCj.append(bCdh)
        ...
        bCo=bCo+5
        bCv=bCv+5
    bCr=list(zip(bCd,bCg,bCp,bCj,bCk,bCm,bCs,bCna))
    bCld=pd.DataFrame(bCr,columns = c)
    Cfd
    ...
    json_records = ld.reset_index().to_json(orient ='records')
    tot_stocks = []
    tot_stocks = json.loads(json_records)
    jon_records = bfd.reset_index().to_json(orient ='records')
    bot_stocks = []
    bot_stocks = json.loads(jon_records)
    jsn_records = fd.reset_index().to_json(orient ='records')
    top_stocks = []
    top_stocks = json.loads(jsn_records)
    #--- technical sector ----\\
    bejon_records = beld.reset_index().to_json(orient ='records')
    bebot_stocks = []
    bebot_stocks = json.loads(bejon_records)
    ejsn_records = efd.reset_index().to_json(orient ='records')
    etop_stocks = []
    etop_stocks = json.loads(ejsn_records)
    #----HEALTH SECTOR DATA MANAGE --- \\\
    bHjon_records = bHld.reset_index().to_json(orient ='records')
    bHbot_stocks = []
    bHbot_stocks = json.loads(bHjon_records)
    Hjsn_records = Hfd.reset_index().to_json(orient ='records')
    Htop_stocks = []
    Htop_stocks = json.loads(Hjsn_records)
    #===FINANCIAL SECTOR DATA MANAGE --\\\
    bFjon_records = bFld.reset_index().to_json(orient ='records')
    bFbot_stocks = []
    bFbot_stocks = json.loads(bFjon_records)
    Fjsn_records = Ffd.reset_index().to_json(orient ='records')
    Ftop_stocks = []
    Ftop_stocks = json.loads(Fjsn_records)
    #---COMMUNICATION SERVICE DATA MANAGE--\\\
    bCjon_records = bCld.reset_index().to_json(orient ='records')
    bCbot_stocks = []
    bCbot_stocks = json.loads(bCjon_records)
    Cjsn_records = Cfd.reset_index().to_json(orient ='records')
    Ctop_stocks = []
    Ctop_stocks = json.loads(Cjsn_records)

    return render(request,'market.html',{'tot_stocks' : tot_stocks,
                                         'top_stocks' : top_stocks,
                                         'bot_stocks' : bot_stocks,
                                         'bebot_stocks' : bebot_stocks,
                                         'etop_stocks' : etop_stocks ,
                                         'bFbot_stocks' : bFbot_stocks,
                                         'Ftop_stocks' : Ftop_stocks ,
                                         'bHbot_stocks' : bHbot_stocks,
                                         'Htop_stocks':Htop_stocks,
                                         'bCbot_stocks' : bCbot_stocks,
                                         'Ctop_stocks':Ctop_stocks})

#def fix(request):
    #return render(request,'fixre.html')
@csrf_exempt
def searchbar(request):
    pf=request.user.profile.profile_image
    user=request.user.username
    if user == 'AnonymousUser':
        user='guest'
    try:
    ...
    return render(request, "sres.html", context={
                                                    'Symbol':Symbol,
                                                    'Name':Name,
                                                    'Last_Sale':Last_Sale,
                                                    'Net_Change':Net_Change,
...
@csrf_exempt
def pay(request):
    stock = request.POST.get('stick')  # Assume this function fetches data for the given ticker
    price= request.POST.get('spri')
    compname= request.POST.get('scom')
    return render(request,'pay.html', {'stock':stock, 
                                       'price':price,
                                       'compname':compname })

def order(request):
    ticker=request.POST.get('fticker')
    cname=request.POST.get('fcname')
    pric=request.POST.get('fprice')
    quan=request.POST.get('fquan')
    price=float(pric)
    quant=int(quan)
    tprice=(price*quant)/66
    ...
    return render(request,'bill.html',{'ticker':ticker,
                                      'cname':cname,
                                       'price':price,
                                       'quan':quan,
                                       'tprice':tprice,
...
def payment_success(request):
    a=request.POST.get('fticker')
    b=PayPalIPN.objects.latest('payer_email')
    print(b)
    return render(request,'payment_success.html',{'a':a,
                                                  'b':b})
...
@csrf_exempt
def otpe(request):
    a=request.POST.get('pmail')
    b=request.POST.get('puser')
    
    from_address = "nightmare9412xgmail.com"
    to_address = a
    ...
    # Credentials
    username = 'nightmare9412x@gmail.com'  
    password = 'zvkjdmldxooacyfq'
    ...
    server.quit()
    obj=otp(user=b,email=a,otp=html)
    obj.save()
    
    
    return render(request,'otbverification.html')

@csrf_exempt
def checkotp(request):
    a=request.POST.get('puser')
    b=request.POST.get('ppass')
    c=request.POST.get('pa')
    d=request.POST.get('pb')
    e=request.POST.get('pc')
    f=request.POST.get('pd')
    u=User.objects.get(username=a)
    otpes=otp.objects.get(user=a)
    o=c+d+e+f
    oi=int(o)
